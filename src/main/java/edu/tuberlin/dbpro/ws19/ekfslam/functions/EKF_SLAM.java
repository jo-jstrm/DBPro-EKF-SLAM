...
public class EKF_SLAM extends RichFlatMapFunction<KeyedDataPoint, KeyedDataPoint> {
    ...
    public static boolean fastPredict = false;
    ...
    public static Tuple2<DoubleMatrix2D, DoubleMatrix2D> predict(ValueState<Tuple3<DoubleMatrix2D, DoubleMatrix2D, Long>> valueState, KeyedDataPoint<Tuple3> inputPoint) throws IOException {
        if(fastPredict){
            ...
            DoubleMatrix2D estimatedCov = two.assign(Rtx, (v, v1) -> v + v1);
            //System.out.println("estimatedCov " + estimatedCov);
            valueState.value().f0.set(0,0, estimatedMu.get(0,0));
            valueState.value().f0.set(1,0, estimatedMu.get(1,0));
            valueState.value().f0.set(2,0, estimatedMu.get(2,0));

            valueState.value().f1.set(0,0, estimatedCov.get(0,0));
            valueState.value().f1.set(1,0, estimatedCov.get(1,0));
            valueState.value().f1.set(2,0, estimatedCov.get(2,0));
            valueState.value().f1.set(0,1, estimatedCov.get(0,1));
            valueState.value().f1.set(1,1, estimatedCov.get(1,1));
            valueState.value().f1.set(2,1, estimatedCov.get(2,1));
            valueState.value().f1.set(0,2, estimatedCov.get(0,2));
            valueState.value().f1.set(1,2, estimatedCov.get(1,2));
            valueState.value().f1.set(2,2, estimatedCov.get(2,2));

            return Tuple2.of(valueState.value().f0, valueState.value().f1);
        }else{
            //Given inputs by the saved value State
            //Double x_prev = valueState.value().f0.get(0,0);
            //Double y_prev = valueState.value().f0.get(1,0);
