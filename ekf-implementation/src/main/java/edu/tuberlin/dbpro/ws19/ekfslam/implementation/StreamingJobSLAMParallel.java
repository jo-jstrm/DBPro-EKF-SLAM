...
public class StreamingJobSLAMParallel {
    public static void main(String[] args) throws Exception {
        ...
        DataStream<KeyedDataPoint> fullData = env.readTextFile("src/main/resources/01_odo_laser_SLAM.csv").setParallelism(1)
                .map(new ParseData()).setParallelism(1)
                .keyBy("key")
                .flatMap(new EKF_SLAM()).setParallelism(4);

        fullData.map(new getTuple3())
                .writeAsCsv("src/main/resources/00_Result_SLAM.csv", FileSystem.WriteMode.OVERWRITE, "\n", ";").setParallelism(1);
